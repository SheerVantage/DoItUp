...
onmessage = async ({data})=>{
  ...
  if(data.action == 'init_schema'){
    ...
    // var row =  fields.reduce(function(obj, field, index) {
    //   obj[field] = data.values[index];
    //   return obj;
    // }, {})
    postMessage({entity:data.entity, iid, action:data.action, row:entry})
  }
  else if(data.action == 'select'){
    rows = select(data.table || data.entity, data.fields, data.conditions, data.orders, data.log)
    // log(rows)
    postMessage({entity:data.entity, rows, action:data.action, sql})
  }
...
function createTrigger(table, fields){
  let sql = `CREATE TRIGGER Date_Modified_${table}
...
let sql
function select(table, fields = '*', conditions = '1', orders = 1, doLog = false){
  // debugger
  if(!db) return
